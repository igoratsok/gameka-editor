...
void MainController::openNewProject(std::string nomeJogo, std::string nomePasta, int preProjectId) {
    GameData* gameData;
    gameData = GameData::getInstance();

    createGameDataInstance(preProjectId, nomeJogo, nomePasta, gameData);
...
void MainController::createGameDataInstance(int preProjectId, std::string gameName, std::string folderName, GameData *gameData) {
    ...
    if(preProjectId == 0) {
        ...
        gameData->filePath = QDir::currentPath().toStdString().append(std::string("/")).append(*filePath);
    } else {
        if(!QDir("projects/").exists()) {
            QDir().mkdir("projects/");
        }
